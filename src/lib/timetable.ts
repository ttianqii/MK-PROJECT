...
/** A course and every section offered for it — for the add-subject picker. */
...
/** Group offered sections by course code (sorted by code). */
export function groupCourses(sections: PlanSection[]): CourseGroup[] {
  const byCode = new Map<string, CourseGroup>();
  for (const s of sections) {
    let g = byCode.get(s.courseCode);
    if (!g) {
      g = { courseCode: s.courseCode, courseName: s.courseName, sections: [] };
      byCode.set(s.courseCode, g);
    }
    g.sections.push(s);
  }
